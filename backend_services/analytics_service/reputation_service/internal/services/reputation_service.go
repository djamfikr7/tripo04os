...
func NewReputationService(db *gorm.DB, logger *zap.Logger) *ReputationService {
...
func (s *ReputationService) CreateReview(ctx context.Context, review *models.Review) (*models.Review, error) {
...
func (s *ReputationService) GetReview(ctx context.Context, id uuid.UUID) (*models.Review, error) {
...
func (s *ReputationService) GetReviewsByUser(ctx context.Context, userID uuid.UUID, limit int) ([]models.Review, error) {
...
func (s *ReputationService) GetReviewsByTrip(ctx context.Context, tripID uuid.UUID) ([]models.Review, error) {
	var reviews []models.Review
	err := s.db.WithContext(ctx).Where("trip_id = ?", tripID).Order("created_at DESC").Find(&reviews).Error
	return reviews, err
}

func (s *ReputationService) GetReputationScore(ctx context.Context, userID uuid.UUID, userType string) (*models.ReputationScore, error) {
...
func (s *ReputationService) UpdateReviewStatus(ctx context.Context, id uuid.UUID, status models.RatingStatus) error {
	result := s.db.WithContext(ctx).Model(&models.Review{}).Where("id = ?", id).Update("status", status)
	return result.Error
}

func (s *ReputationService) createInitialReputation(ctx context.Context, userID uuid.UUID, userType string) (*models.ReputationScore, error) {
...
func (s *ReputationService) recalculateReputation(ctx context.Context, userID uuid.UUID, userType string) error {
...
func (s *ReputationService) getRecentReviews(ctx context.Context, userID uuid.UUID, userType string) ([]models.Review, error) {
	var reviews []models.Review
...
func (s *ReputationService) calculateScores(score *models.ReputationScore, reviews []models.Review) {
	if len(reviews) == 0 {
		return
	}
...
func (s *ReputationService) updateComponentScores(score *models.ReputationScore, review models.Review) {
	const weight = 0.05
	const decay = 0.95
...
func (s *ReputationService) calculateTrustScore(score *models.ReputationScore) float64 {
...
func (s *ReputationService) recordHistory(ctx context.Context, userID uuid.UUID, userType string, oldScore, newScore float64, reason string, relatedReviewID *uuid.UUID) error {
...
func (s *ReputationService) GetReputationHistory(ctx context.Context, userID uuid.UUID, limit int) ([]models.ReputationHistory, error) {
...
func (s *ReputationService) UpdateTripStats(ctx context.Context, userID uuid.UUID, userType string, completed, cancelled bool) error {
	var score models.ReputationScore
	err := s.db.WithContext(ctx).Where("user_id = ? AND user_type = ?", userID, userType).First(&score).Error
	if err != nil {
		if err == gorm.ErrRecordNotFound {
...
func (s *ReputationService) GetTopRatedUsers(ctx context.Context, userType string, limit int) ([]models.ReputationScore, error) {
...
func (s *ReputationService) GetReputationStats(ctx context.Context) (map[string]interface{}, error) {
	var stats struct {
		TotalReviews    int64
		TotalUsers      int64
		TotalDrivers    int64
